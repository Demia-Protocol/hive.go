...
func TestConcurrentCreateDelete(t *testing.T) {
	...
	// create badger DB
	mappedDbMissingMessageStorage, err := mapdb.NewMapDB().WithRealm([]byte("TestRealm"))
	//badgerDBMissingMessageStorage, err := testutil.BadgerDB(t)
	require.NoError(t, err)
	mappedDbMetadataStorage, err := mapdb.NewMapDB().WithRealm([]byte("TestRealmMetadata"))
	//badgerDBMetadataStorage, err := testutil.BadgerDB(t)
	require.NoError(t, err)

	// create ObjectStorage instances
	missingMessageStorage := objectstorage.New(mappedDbMissingMessageStorage, testObjectFactory)
	metadataStorage := objectstorage.New(mappedDbMetadataStorage, testObjectFactory)
	...
	for i := 0; i < objectCount; i++ {
		...
		wp.Submit(func() {
			obj, err := metadataStorage.Store(newTestObject(messageIDString, x))
			if err != nil {
				t.Error(err)
			}
			obj.Release()
...
func BenchmarkStore(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		obj, err := objects.Store(newTestObject("Hans"+strconv.Itoa(i), uint32(i)))
		if err != nil {
			b.Error(err)
		}
		obj.Release()
...
func TestDelete(t *testing.T) {
	...
	objects := objectstorage.New(storage, testObjectFactory)
	obj, err := objects.Store(newTestObject("Hans", 33))
	if err != nil {
		t.Error(err)
	}
	obj.Release()
...
func TestEvictionBug(t *testing.T) {
	...
	for i := 0; i < testCount; i++ {
		go func(i int) {
			obj, err := objects.Store(newTestObject(fmt.Sprintf("%v", i), 0))
			if err != nil {
				t.Error(err)
			}
			obj.Release()
